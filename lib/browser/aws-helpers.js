...
function findDatabaseInstanceTypes(root) {
    const instanceTypes = [];
    const lists = Array.from(root.querySelectorAll("ul.itemizedlist"));

    for (const list of lists) {
        const items = list.querySelectorAll("li");
        for (const item of items) {
            const type = item.querySelector("p > .topcom");
            if (type !== null) {
                instanceTypes.push(type.textContent.trim());
            }
        }
    }
