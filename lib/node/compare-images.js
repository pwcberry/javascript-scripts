...
async function compareImages(srcPath, tgtPath, threshold) {
    let srcImg, tgtImg;
    try {
        await using handle = await open(srcPath, "r");
        const bytes = await handle.readFile();
        srcImg = new Sharp(bytes);
    } catch (e) {
        throw new Error(`Cannot read source image: ${srcPath} (${e.message})`);
    }
    try {
        await using handle = await open(tgtPath, "r");
        const bytes = await handle.readFile();
        tgtImg = new Sharp(bytes);
    } catch (e) {
        throw new Error(`Cannot read target image: ${tgtPath} (${e.message})`);
    }

    // Resize target to source dimensions for a fair pixel diff
    const resized = tgtImg.clone().resize(srcImg.width, srcImg.height, { kernel: "nearest" });
    const { width, height, diff } = imageDiff(srcImg, resized, threshold);
