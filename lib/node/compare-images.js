...
async function compareImages(srcPath, tgtPath, threshold) {
    let srcImg, tgtImg;
    try {
        srcImg = await Jimp.read(srcPath);
    } catch (e) {
        throw new Error(`Cannot read source image: ${srcPath} (${e.message})`);
    }
    try {
        tgtImg = await Jimp.read(tgtPath);
    } catch (e) {
        throw new Error(`Cannot read target image: ${tgtPath} (${e.message})`);
    }

    // Resize target to source dimensions for a fair pixel diff
    const { width, height } = srcImg.bitmap;
    const resized = tgtImg.clone().resize({ w: width, h: height, mode: ResizeStrategy.NEAREST_NEIGHBOR });

    // Jimp.diff returns { percent: 0–1, image: <diff bitmap> }
    const imageDiff = diff(srcImg, resized, threshold);

    return {
        score: imageDiff.percent, // 0 = identical, 1 = completely different
        similar: imageDiff.percent <= threshold,
        width,
        height,
    };
}
...
async function exportReport(results, outputDir) {
    const similar = results.filter((r) => r.similar).map((r) => ({ ...r, score: formatScore(r.score) }));
    const different = results.filter((r) => !r.similar).map((r) => ({ ...r, score: formatScore(r.score) }));
    const data = {
        similar,
        different,
    };
