...
async function compareImages(srcPath, tgtPath, threshold) {
    ...
    try {
        tgtImg = await sharp(tgtPath).toBuffer({ resolveWithObject: true });
    } catch (e) {
        throw new Error(`Cannot read target image: ${tgtPath} (${e.message})`);
    }

    // TODO: Resize images for better match?
    ...
    const { width, height, diff } = imageDiff(sourceImage, targetImage, {
